...
import { 
  ...
  Palette, Plus, Type, Square, Circle, BringToFront, SendToBack, MousePointer2
} from 'lucide-react';
        ...
        <div className="flex-1 overflow-hidden relative">
            {isLoadingData ? (
              ...
              <div className="w-full h-full overflow-y-auto scrollbar-thin">
                ...
                {activeView === 'brief' && <BriefSlideGenerator setTasks={setTasks} editData={editTaskData} setEditData={setEditTaskData} activeUser={activeUser} apiUrl={API_URL} setActiveView={setActiveView} theme={theme} />}
                {activeView === 'board' && <BoardView tasks={filteredTasks} setTasks={setTasks} activeUser={activeUser} onEditBrief={(t) => { setEditTaskData(t); setActiveView('brief'); }} onViewBrief={setViewingTask} apiUrl={API_URL} theme={theme} />}
...
// ============================================================================
// 1. BRIEF VIEWER MODAL (Graphic Viewer)
// ============================================================================
function BriefViewerModal({ task, onClose, activeUser, setTasks, apiUrl, theme }) {
  ...
  const handleSubmitWork = async () => {
    ...
    try {
      ...
      alert("Error submitting work (Mock Mode)");
      onClose();
    } finally {
      ...
      <div className="bg-slate-900 border border-slate-700 rounded-3xl w-full max-w-6xl h-full max-h-[90vh] flex flex-col shadow-2xl overflow-hidden relative">
        ...
        <div className="flex flex-col lg:flex-row flex-1 overflow-hidden bg-slate-900">
          ...
          <div className="lg:w-[35%] bg-slate-950/50 border-r border-slate-800 p-8 overflow-y-auto scrollbar-thin flex flex-col gap-8">
            <div>
              ...
              <div className="space-y-6">
                ...
                <div>
                  <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-2">Required Sizes</p>
                ...
                {task.refLink && (
                  <div>
                    <p className="text-[10px] text-slate-500 font-bold uppercase tracking-wider mb-2">Reference Link</p>
          ...
          <div className="lg:w-[65%] flex flex-col relative overflow-hidden bg-slate-100">
            ...
            <div className="bg-slate-900 border-t border-slate-800 p-6 flex flex-col shrink-0 max-h-64 overflow-y-auto">
              <div className="mb-4">
                <p className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2 flex items-center gap-2"><MessageCircle className="w-3 h-3"/> Social Media Caption</p>
                ...
                <div className={`mt-2 ${theme.lightBg} border ${theme.border} rounded-2xl p-4 shadow-xl shrink-0 backdrop-blur`}>
                  ...
                  <div className="flex gap-2">
                    <div className="relative flex-1">
                      ...
                      <input 
                        type="url" 
                        placeholder="Paste link here (Google Drive / Frame.io)..." 
                        value={driveLink}
...
// ============================================================================
// 4. FREE-FORM CANVAS BRIEF GENERATOR
// ============================================================================
function BriefSlideGenerator({ setTasks, editData, setEditData, activeUser, setActiveView, apiUrl, theme }) {
  ...
  const addElement = (type) => {
    ...
    if(type === 'text') { newEl.text = 'New Text'; newEl.color = '#000000'; newEl.fontSize = 24; newEl.fontWeight = 'bold'; }
    ...
    <div className="flex h-full w-full overflow-hidden bg-slate-900 relative">
      <div className={`h-full overflow-y-auto p-6 flex flex-col items-center bg-slate-900/60 transition-all w-2/3`}>
            ...
            <div className="bg-slate-900 p-3 flex gap-4 items-center border-b border-slate-800">
              {selectedEl.type === 'text' && (
                <>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <input type="number" value={selectedEl.fontSize} onChange={e => updateSelectedElement({fontSize: parseInt(e.target.value)})} className="w-16 bg-slate-950 border border-slate-700 text-white text-xs px-2 py-1.5 rounded outline-none"/>
                  </div>
                </>
              )}
              {selectedEl.type === 'image' && (
                <>
                  ...
                  <div className="flex items-center gap-2 text-[10px] text-slate-400">
                    W: <input type="number" value={selectedEl.width} onChange={e => updateSelectedElement({width: parseInt(e.target.value)})} className="w-16 bg-slate-950 border border-slate-700 text-white px-2 py-1.5 rounded outline-none"/>
                  </div>
                </>
              )}
              {(selectedEl.type === 'rect' || selectedEl.type === 'circle') && (
                <>
                  ...
                  <div className="flex items-center gap-2 text-[10px] text-slate-400">
                    W: <input type="number" value={selectedEl.width} onChange={e => updateSelectedElement({width: parseInt(e.target.value)})} className="w-16 bg-slate-950 border border-slate-700 text-white px-2 py-1.5 rounded outline-none"/>
                    H: <input type="number" value={selectedEl.height} onChange={e => updateSelectedElement({height: parseInt(e.target.value)})} className="w-16 bg-slate-950 border border-slate-700 text-white px-2 py-1.5 rounded outline-none"/>
                  </div>
                </>
              )}
            </div>
          ...
          >
            {!elements.length && (
              <div className="absolute inset-0 flex flex-col items-center justify-center text-slate-400 opacity-50 pointer-events-none">
                ...
                <p className="font-bold">Add elements to design your brief</p>
              </div>
            )}
            
            {elements.map(el => {
              const style = {
                position: 'absolute', left: el.x, top: el.y, zIndex: el.zIndex || 1,
                cursor: 'grab', userSelect: 'none', border: selectedId === el.id ? '2px solid #3b82f6' : '2px solid transparent'
              };
              if(isDragging && selectedId === el.id) style.cursor = 'grabbing';

              return (
                <div key={el.id} style={style} onMouseDown={(e) => handleMouseDown(e, el.id)}>
                  {el.type === 'text' && <div style={{color: el.color, fontSize: el.fontSize, fontWeight: el.fontWeight}} className="whitespace-pre-wrap">{el.text}</div>}
                  {el.type === 'rect' && <div style={{width: el.width, height: el.height, backgroundColor: el.fill}}></div>}
                  {el.type === 'circle' && <div style={{width: el.width, height: el.height, backgroundColor: el.fill, borderRadius: '50%'}}></div>}
                  {el.type === 'image' && <img src={el.url} style={{width: el.width, height: el.height, objectFit: 'contain'}} alt="Canvas Element" draggable={false}/>}
                </div>
              );
            })}
          </div>
        ...
        <div className="w-full max-w-[800px] mt-6 bg-slate-950 border border-slate-800 rounded-2xl p-6">
            ...
            <textarea name="caption" value={formData.caption} onChange={handleInputChange} rows={3} className={`w-full bg-slate-900 border border-slate-700 p-3 rounded-xl text-xs text-white ${theme.outline} outline-none`} placeholder="Type social media caption here..."/>
        </div>
      </div>
      ...
      <div className="w-1/3 h-full bg-slate-950 border-l border-slate-800 overflow-y-auto p-6 scrollbar-thin">
        ...
        <div className="bg-amber-500/10 border border-amber-500/50 p-4 rounded-xl mb-6 text-xs text-amber-400 font-medium leading-relaxed">
           <strong className="block text-amber-500 font-black mb-1">Artwork Request Policy</strong>
           Please allow 1 week in advance. <span className="text-rose-400 font-black">Urgent</span> requests will be locked and require Manager approval.
        </div>
           ...
           <div className="space-y-5 bg-slate-900/50 p-5 rounded-2xl border border-slate-800/80">
              ...
              <div>
                ...
                <div className="flex gap-2 mb-2">
                  ...
                  <button onClick={handleAddCustomSize} className={`${theme.bg} text-white px-4 rounded-xl font-bold text-xs`}><Plus className="w-4 h-4"/></button>
                </div>
                <div className="flex flex-wrap gap-2">
...
// 5. KANBAN BOARD
// ----------------------------------------------------------------------
function BoardView({ tasks, setTasks, activeUser, apiUrl, onViewBrief, onEditBrief, theme }) {
  ...
  return (
    <div className="h-full flex flex-col p-6 lg:p-8 animate-in fade-in duration-200 overflow-y-auto">
      <div className="mb-6 flex justify-between items-center border-b border-slate-800 pb-4 shrink-0">
        <div>
          <h2 className="text-xl font-black text-white">Creative Pipeline</h2>
          <p className="text-xs text-slate-500 mt-1">Click on a card to view brief and submit work.</p>
        </div>
      </div>
      ...
      {upcomingTasks.length > 0 && (
        <div className="mb-8 shrink-0">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            {upcomingTasks.map(t => (
              <div key={`upc-${t.id}`} className="bg-slate-950 border border-slate-800 p-4 rounded-xl flex items-center justify-between shadow-sm cursor-pointer hover:border-slate-600" onClick={() => onViewBrief(t)}>
                ...
                <div className="text-right shrink-0 ml-4">
                  ...
                  <span className="text-xl font-black text-rose-400">{new Date(t.date).toLocaleDateString('en-GB', { day: 'numeric', month: 'short' })}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* Grid Layout without horizontal scroll */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-5 gap-6 items-start pb-10">
            ...
            <div key={stage} className={`bg-slate-950/40 border rounded-3xl flex flex-col ${stage === 'Incoming Requests' ? 'border-amber-900/50' : stage === 'Open Pool' ? 'border-blue-900/50' : 'border-slate-800'}`}>
              ...
              <div className={`p-4 border-b flex justify-between items-center rounded-t-3xl ${stage === 'Incoming Requests' ? 'bg-amber-950/40 border-amber-900/50' : stage === 'Open Pool' ? 'bg-blue-950/40 border-blue-900/50' : 'bg-slate-950/80 border-slate-800'}`}>
                <div className="flex items-center gap-2">
                  ...
                  <h4 className={`text-[11px] font-black uppercase tracking-wider ${stage === 'Incoming Requests' ? 'text-amber-500' : stage === 'Open Pool' ? 'text-blue-400' : 'text-slate-300'}`}>{stage}</h4>
                </div>
                <span className="bg-slate-900 border border-slate-800 text-[10px] px-2 py-0.5 rounded-full text-slate-400 font-black">{stageTasks.length}</span>
              </div>

              <div className="p-3 flex-col space-y-3">
                {stageTasks.map(task => (
                  <div key={task.id} className={`bg-slate-900 p-4 rounded-2xl border border-slate-800/80 hover:border-slate-500 hover:shadow-lg transition-all group relative cursor-pointer`} onClick={() => onViewBrief(task)}>
                    ...
                    <h5 className="text-sm font-bold text-white mb-2 leading-snug">{task.topic}</h5>
                    ...
                    <div className="bg-slate-950 border border-slate-800 rounded-xl p-3 my-3 text-center">
                       ...
                       <p className="text-xl font-black text-rose-400">{new Date(task.date).toLocaleDateString('en-GB', { day: '2-digit', month: 'short' })}</p>
                    </div>
                    ...
                    <div onClick={(e) => e.stopPropagation()}>
                      ...
                      {stage === 'Open Pool' && (
                         <div className="mt-2">
                           {(activeUser.Role === 'Graphic' || activeUser.Role === 'Editor' || activeUser.Role === 'SuperAdmin') && (task.designer === 'Unassigned' || !task.designer) && (
                             <button onClick={() => claimTask(task.id)} className={`w-full py-2 ${theme.lightBg} ${theme.text} text-xs font-black rounded-xl border ${theme.border} ${theme.hover} hover:text-white transition-colors`}>
                               Pick up Task
                             </button>
                           )}
                         </div>
                      ...
                      {sIdx > 1 && (task.designer === activeUser.Name || activeUser.Role === 'SuperAdmin') && (
...
// ----------------------------------------------------------------------
// 7. TABLE VIEW (เพิ่มกรองดูคอนเทนต์รายเดือน)
// ----------------------------------------------------------------------
  ...
  const displayTasks = useMemo(() => {
    ...
    if (filterMonth !== 'ALL') {
      list = list.filter(t => {
        ...
        return !isNaN(d) && d.getMonth().toString() === filterMonth;
      });
    }
            ...
            <tbody className="text-xs divide-y divide-slate-800/50">
              {displayTasks.map(t => (
                <tr key={t.id} className="hover:bg-slate-900/50">
                  ...
                  <td className={`p-4 font-bold ${theme.text}`}>{new Date(t.date).toLocaleDateString('en-GB')}</td>
                  <td className="p-4 font-bold text-white">{t.project}</td>
                  <td className="p-4 text-slate-300">{t.topic}</td>
                  <td className="p-4 text-slate-400">{t.designer || 'Unassigned'}</td>
                  <td className="p-4">
                    <span className={`px-2 py-1 rounded-md text-[10px] font-black ${t.status === 'Published' ? 'bg-emerald-950 text-emerald-400 border border-emerald-800' : 'bg-slate-800 text-slate-300'}`}>{t.status}</span>
                  </td>
                </tr>
              ))}
              {displayTasks.length === 0 && (
                <tr><td colSpan="6" className="text-center p-8 text-slate-500">No content found for this selection.</td></tr>
              )}
            </tbody>
          </table>
        </div>
...
// ----------------------------------------------------------------------
// 8. TEAM & ADMIN VIEW (เพิ่ม/ลบ Role ได้)
// ----------------------------------------------------------------------
function TeamAdminView({ teamMembers, setTeamMembers, activeUser, permissions, setPermissions, apiUrl, theme }) {
  ...
  return (
    <div className="p-6 lg:p-8 space-y-8 relative animate-in fade-in h-full overflow-y-auto">
